...
simpleCad.controller('SimpleCadController', ['$scope', function($scope){
  ...
  $scope.sliders = {
    ...
    x:            {label: 'X',                  min: -1, max: 1,    step: 0.01,  value: 0}, 
    y:            {label: 'Y',                  min: -1, max: 1,    step: 0.01,  value: 0}, 
    scaleX:       {label: 'Scale X',            min: -1, max: 1,    step: 0.01,  value: 0.5}, 
    scaleY:       {label: 'Scale Y',            min: -1, max: 1,    step: 0.01,  value: 0.5}, 
    theta:        {label: 'Rotation (&Theta;)', min: 0, max: 360,  step: 1,  value: 0}, 
    phi:          {label: 'Rotation (&Phi;)',   min: 0, max: 360,  step: 1,  value: 0}
  };
  ...
  $scope.$watch(getInputs, function(newVal, oldVal){
    if (init)
    {
      ...
      position = {x: newVal.x, y: newVal.y};
...
function render() {
  ...
  projectionMatrix = mult(translate(position.x,position.y,0), mult(scalem(scale.x,scale.y,1), ortho(left, right, bottom, ytop, near, far)));
